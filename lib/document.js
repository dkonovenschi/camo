...
class Document extends BaseDocument {
    ...
    save() {
        ...
        return Promise.all(preValidatePromises).then(function () {
            ...
            _.keys(that._schema).forEach(function (key) {
                ...
                if (isReferenceable(that[key]) ||            // isReferenceable OR
                    ...
                    if (isArray(that[key])) {
                        ...
                        if (connection.isNativeId(that[key])) {
            ...
            return connection.save(that.collectionName(), that._id, toUpdate);
        }).then(function (id) {
            if (that._id === null) {
                that._id = id;
            }
    ...
    delete() {
        ...
        return Promise.all(preDeletePromises).then(function () {
            return connection.delete(that.collectionName(), that._id);
    ...
    static deleteOne(query) {
        return connection.deleteOne(this.collectionName(), query);
    }
    ...
    static deleteMany(query) {
        ...
        return connection.deleteMany(this.collectionName(), query);
    }
    ...
    static findOne(query, options) {
        ...
        return connection.findOne(this.collectionName(), query)
            .then(function (data) {
                if (!data) {
                    return null;
                }
    ...
    static findOneAndUpdate(query, values, options) {
        ...
        return connection.findOneAndUpdate(this.collectionName(), query, values, options)
            .then(function (data) {
                if (!data) {
                    return null;
                }
    ...
    static findOneAndDelete(query, options) {
        ...
        return connection.findOneAndDelete(this.collectionName(), query, options);
    }
    ...
    static find(query, options) {
        ...
        return connection.find(this.collectionName(), query, options)
            .then(function (datas) {
    ...
    static count(query) {
        const that = this;
        return connection.count(this.collectionName(), query);
    }
    ...
    static createIndexes() {
        ...
        _.keys(instance._schema).forEach(function (k) {
            if (instance._schema[k].unique) {
                connection.createIndex(that.collectionName(), k, {unique: true});
            }
        });
    ...
    static clearCollection() {
        return connection.clearCollection(this.collectionName());
    }
