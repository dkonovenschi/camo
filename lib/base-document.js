...
class BaseDocument {
    constructor() {
        ...
        this._schema = {                            // Defines document structure/properties
            _id: {type: connection.nativeIdType()},     // Native ID to backend database
        };
    ...
    setConnection() {
        if (typeof this.connectionName === 'undefined') {
            this.connection = DB().getDefaultClient();
        } else {
            this.connection = DB().getClient(this.connectionName);
        }
    }
    ...
    validate() {
        ...
        _.keys(that._schema).forEach(function (key) {
            ...
            if (!isValidType(value, that._schema[key].type, that.connection)) {
    ...
    static populate(docs, fields) {
        ...
        _.keys(anInstance._schema).forEach(function (key) {
            ...
            else if ((isString(anInstance[key]) || connection.isNativeId(anInstance[key])) &&
        ...
        keys.forEach(function (k) {
            ...
            documents.forEach(function (d) {
                ids[k][connection.toCanonicalId(d._id)] = [].concat(d[k]);     // Handles values and arrays
            ...
            let p = type.find({'_id': {$in: keyIds}}, {populate: false})
                .then(function (dereferences) {
                    ...
                    _.keys(ids[key]).forEach(function (k) {
                        ...
                        let doc;
                        documents.forEach(function (d) {
                            if (connection.toCanonicalId(d._id) === k) doc = d;
                        });
                        ...
                        ids[key][k].forEach(function (id) {
                            ...
                            dereferences.forEach(function (d) {
                                if (connection.toCanonicalId(d._id) === connection.toCanonicalId(id)) deref = d;
                            });
