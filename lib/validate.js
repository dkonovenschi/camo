...
const isNativeId = function (n, connection) {
    return connection.isNativeId(n);
};
...
const isType = function (value, type, connection) {
    if (type === String) {
        ...
        return isDocument(value) || connection.isNativeId(value);
    } else if (type.documentClass && type.documentClass() === 'embedded') {
        return isEmbeddedDocument(value);
    } else if (type === connection.nativeIdType()) {
...
const isValidType = function (value, type, connection) {
    // NOTE
    // Maybe look at this: 
    ...
    if (type === Array || isArray(type)) {
        ...
        if (isArray(type) && type.length === 1 && isArray(value)) {
            ...
            for (let i = 0; i < value.length; i++) {
                let v = value[i];
                if (!isType(v, arrayType, connection)) {
                    return false;
                }
            }
    ...
    return isType(value, type, connection);
};
